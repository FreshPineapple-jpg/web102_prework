...
const totalContrubutions = GAMES_JSON.reduce((total, game) => {return total + game.backers}, 0);
const totalRaised = GAMES_JSON.reduce((total,game) => {return total + game.pledged}, 0)
const totalGames = GAMES_JSON.length;
// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = totalContrubutions.toLocaleString();
...
const displayStr = `
    ...
    Currently, ${unfundedGamesCount} ${unfundedGamesCount === 1 ? 'game' : 'games'} remain unfunded.
    We need your help to fund ${unfundedGamesCount === 1 ? 'this' : 'these'} amazing ${unfundedGamesCount === 1 ? 'game' : 'games'}!
`;
...
firstGameImg.alt = firstGame.name;
firstGameImg.classList.add("game-img");
firstGameElement.appendChild(firstGameImg);
...
secondGameImg.alt = secondGame.name;
secondGameImg.classList.add("game-img");
secondGameElement.appendChild(secondGameImg);
